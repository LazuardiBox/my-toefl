...
function extractRoutePath(
  ...
  const resolveExpressionToString = (expr: ts.Expression): string | null => {
    ...
    if (ts.isIdentifier(expr)) {
      return (
        constStrings.get(expr.text) ?? importedStrings.get(expr.text) ?? null
      );
    }
...
export function customRoutesManifestPlugin(): Plugin {
  ...
  return {
    ...
    buildStart() {
      ...
      (
        globalThis as { TSS_ROUTES_MANIFEST?: Record<string, ManifestRoute> }
      ).TSS_ROUTES_MANIFEST = routesManifest;
    },
  };
}
